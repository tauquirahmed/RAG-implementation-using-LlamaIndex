...
def main():
    ...
    if st.button("Select"):
        if option == 'gemini-pro':
            llm = genai.configure(api_key=os.environ.get('GOOGLE_API_KEY'), client_options={"api_endpoint": "generativelanguage.googleapis.com"})
        elif option == 'gpt-4':
            llm = OpenAI(model="gpt-4", temperature=0, max_tokens=256, api_key=os.environ.get('OPENAI_API_KEY'))
